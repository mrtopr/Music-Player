...
export const usePlaylistStore = create((set, get) => ({
    ...
    importPlaylist: (playlistData) => {
        if (!playlistData || !playlistData.name || !Array.isArray(playlistData.songs)) return;
        set(state => {
            const baseName = `[Shared] ${playlistData.name}`;
            let uniqueName = baseName;
            let counter = 1;
            while (state.playlists.some(p => p.name === uniqueName)) {
