...
const ENDPOINTS = {
  ...
  newReleasesAlbums: "/api/search/albums?query=bollywood%20new%20releases%202024&language=hindi",
...
function initializeFullscreenPlayerEnhancements() {
  ...
  if (audio) {
    audio.addEventListener('play', () => {
      ...
      if (window.playerControls && window.playerControls.updatePlayButtons) {
...
async function fetchTrendingSongs(page = 0, append = false) {
  try {
    ...
    if (!response.ok) {
      console.error('❌ Trending songs API response not OK:', response.status, response.statusText);
...
function showTrendingPage() {
  ...
  if (trendingPage) {
    ...
    loadTrendingPage(0); // API uses 0-based indexing
  }
...
function showLatestPage() {
  ...
  if (latestPage) {
    ...
    // Reset pagination and load first page
    latestCurrentPage = 1;
    loadLatestPage(0); // API uses 0-based indexing
  }
...
function showArtistsPage() {
  ...
  if (artistsPage) {
    ...
    loadArtistsPage(0); // API uses 0-based indexing
  } else {
    showNotification('Artists page coming soon! 🎤', 'info');
  }
}
...
// Dedicated page loading functions
let trendingPageNum = 0;
let latestPageNum = 0;
let trendingPageSongs = [];
let latestPageAlbums = [];

async function loadTrendingPage(page = 0, appendMode = false) {
  console.log('loadTrendingPage called with page:', page, 'appendMode:', appendMode);
  const container = document.getElementById('trending-page-container');
  
  if (!container) {
    console.error('trending-page-container not found');
    return;
  }
  ...
  if (!appendMode) {
    ...
    currentPageSongs = [];
    allLoadedSongs = [];
    currentPageType = 'trending';
  }
  
  try {
    const url = `${API_BASE_URL}${ENDPOINTS.trendingSongs}&page=${page}&limit=20`; // Increased limit for better experience
    ...
    if (data?.success && data?.data?.results && data.data.results.length > 0) {
      const songs = data.data.results.map(s => processSongData(s));
      console.log('Processed songs:', songs.length);
      
      // Add songs to current page and all loaded songs
      if (appendMode) {
        currentPageSongs = [...currentPageSongs, ...songs];
        allLoadedSongs = [...allLoadedSongs, ...songs];
      } else {
        currentPageSongs = songs;
        allLoadedSongs = songs;
      }
      
      // Render songs for current page
      songs.forEach((song, index) => {
        const globalIndex = appendMode ? allLoadedSongs.length - songs.length + index : index;
        const card = createSongCard(song, globalIndex, false, true);
        container.appendChild(card);
      });
      
      // Update pagination UI (unlimited pages)
      updatePaginationUI('trending', trendingCurrentPage);
      
      // Update global songs array for playback
      window.songs = allLoadedSongs;
      
      console.log('Total loaded songs:', allLoadedSongs.length);
      
      if (!appendMode) {
        showNotification(`Loaded ${songs.length} trending songs! 🔥`, 'info');
      }
    } else {
      console.log('No more songs available for page:', page);
      if (!appendMode) {
        container.innerHTML = '<div class="empty-state"><i class="bi bi-exclamation-triangle"></i><h3>No trending songs found</h3><p>Please try again later.</p></div>';
      } else {
        showNotification('No more songs available', 'info');
      }
    }
  } catch (error) {
...
async function loadLatestPage(page = 0, appendMode = false) {
  console.log('loadLatestPage called with page:', page, 'appendMode:', appendMode);
  ...
  if (!appendMode) {
    ...
    currentPageSongs = [];
    allLoadedSongs = [];
    currentPageType = 'latest';
  }
  
  try {
    const response = await fetch(
      `${API_BASE_URL}${ENDPOINTS.newReleasesAlbums}&page=${page}&limit=20`
    );
    ...
    if (data?.success && data?.data?.results && data.data.results.length > 0) {
      ...
      // Add albums to current page and all loaded songs
      if (appendMode) {
        currentPageSongs = [...currentPageSongs, ...albums];
        allLoadedSongs = [...allLoadedSongs, ...albums];
      } else {
        currentPageSongs = albums;
        allLoadedSongs = albums;
      }
      
      // Render albums for current page
      albums.forEach((album, index) => {
        const globalIndex = appendMode ? allLoadedSongs.length - albums.length + index : index;
        const card = createMediaCard(album, 'album');
        container.appendChild(card);
      });
      
      // Update pagination UI (unlimited pages)
      updatePaginationUI('latest', latestCurrentPage);
      
      // Update global songs array for playback (convert albums to songs if needed)
      window.songs = allLoadedSongs;
      
      console.log('Total loaded albums:', allLoadedSongs.length);
      
      if (!appendMode) {
        showNotification(`Loaded ${albums.length} latest releases! 🎵`, 'info');
      }
    } else {
      console.log('No more albums available for page:', page);
      if (!appendMode) {
        container.innerHTML = '<div class="empty-state"><i class="bi bi-exclamation-triangle"></i><h3>No latest releases found</h3><p>Please try again later.</p></div>';
      } else {
        showNotification('No more releases available', 'info');
      }
    }
  } catch (error) {
    console.error('Error loading latest page:', error);
    showNotification('Error loading latest releases', 'error');
    if (!appendMode) {
      container.innerHTML = '<div class="empty-state"><i class="bi bi-exclamation-triangle"></i><h3>Error loading latest releases</h3><p>Please try again later.</p></div>';
    }
  }
}
...
// ======== ENHANCED UNLIMITED PAGINATION SYSTEM ========
let trendingCurrentPage = 1;
let latestCurrentPage = 1;
let artistsCurrentPage = 1;
let trendingTotalPages = 999; // Set to high number for unlimited pagination
let latestTotalPages = 999;
let artistsTotalPages = 999;

// Enhanced pagination state management
let currentPageType = null; // 'trending', 'latest', 'artists'
let currentPageSongs = []; // Songs from current page
let allLoadedSongs = []; // All songs loaded across pages
let currentSongIndexInPage = 0; // Current song index within the page
let isAutoFetchingNextPage = false;

// Pagination utility functions
function updatePaginationUI(type, currentPage, totalPages = 999) {
  ...
  if (prevBtn && nextBtn && pageInfo && paginationContainer) {
    // Update page info - show unlimited pages
    pageInfo.textContent = `Page ${currentPage}`;
    
    // Update button states
    prevBtn.disabled = currentPage <= 1;
    nextBtn.disabled = false; // Never disable next button for unlimited pagination
    
    // Always show pagination for unlimited browsing
    paginationContainer.style.display = 'flex';
  }
}

async function loadPageData(type, page, loadFunction) {
  const container = document.getElementById(`${type}-page-container`);
  if (!container) return;
  ...
  try {
    ...
    showNotification(`Error loading ${type} data`, 'error');
  } finally {
...
// Auto-fetch next page when current page songs are finished
async function autoFetchNextPageIfNeeded() {
  if (!currentPageType || isAutoFetchingNextPage) return;
  
  const currentSongIndex = window.currentSongIndex || 0;
  const totalSongsInCurrentPage = currentPageSongs.length;
  
  // Check if we're near the end of current page songs
  if (currentSongIndex >= totalSongsInCurrentPage - 2) {
    console.log('Near end of current page, auto-fetching next page...');
    isAutoFetchingNextPage = true;
    
    try {
      // Increment page and fetch next page
      if (currentPageType === 'trending') {
        trendingCurrentPage++;
        await loadTrendingPage(trendingCurrentPage - 1, true); // true = append mode
      } else if (currentPageType === 'latest') {
        latestCurrentPage++;
        await loadLatestPage(latestCurrentPage - 1, true);
      } else if (currentPageType === 'artists') {
        artistsCurrentPage++;
        await loadArtistsPage(artistsCurrentPage - 1, true);
      }
      
      showNotification(`Loaded page ${currentPageType === 'trending' ? trendingCurrentPage : currentPageType === 'latest' ? latestCurrentPage : artistsCurrentPage} automatically! 🎵`, 'info');
    } catch (error) {
      console.error('Error auto-fetching next page:', error);
    } finally {
      isAutoFetchingNextPage = false;
    }
  }
}
...
safeAddListener(window, 'DOMContentLoaded', () => {
  ...
  if (trendingPrevBtn) {
    trendingPrevBtn.addEventListener('click', async () => {
      if (trendingCurrentPage > 1) {
        trendingCurrentPage--;
        await loadPageData('trending', trendingCurrentPage - 1, (page) => loadTrendingPage(page, false));
        updatePaginationUI('trending', trendingCurrentPage);
      }
    });
  }
  ...
  if (trendingNextBtn) {
    trendingNextBtn.addEventListener('click', async () => {
      // No limit check for unlimited pagination
      trendingCurrentPage++;
      await loadPageData('trending', trendingCurrentPage - 1, (page) => loadTrendingPage(page, false));
      updatePaginationUI('trending', trendingCurrentPage);
    });
  ...
  // Latest pagination
  const latestPrevBtn = document.getElementById('latest-prev-btn');
  const latestNextBtn = document.getElementById('latest-next-btn');
  
  if (latestPrevBtn) {
    latestPrevBtn.addEventListener('click', async () => {
      if (latestCurrentPage > 1) {
        latestCurrentPage--;
        await loadPageData('latest', latestCurrentPage - 1, (page) => loadLatestPage(page, false));
        updatePaginationUI('latest', latestCurrentPage);
      }
    });
  }
  ...
  if (latestNextBtn) {
    latestNextBtn.addEventListener('click', async () => {
      // No limit check for unlimited pagination
      latestCurrentPage++;
      await loadPageData('latest', latestCurrentPage - 1, (page) => loadLatestPage(page, false));
      updatePaginationUI('latest', latestCurrentPage);
  ...
  // Artists pagination
  const artistsPrevBtn = document.getElementById('artists-prev-btn');
  const artistsNextBtn = document.getElementById('artists-next-btn');
  
  if (artistsPrevBtn) {
    artistsPrevBtn.addEventListener('click', async () => {
      if (artistsCurrentPage > 1) {
        artistsCurrentPage--;
        await loadPageData('artists', artistsCurrentPage - 1, (page) => loadArtistsPage(page, false));
        updatePaginationUI('artists', artistsCurrentPage);
      }
    });
  }
  
  if (artistsNextBtn) {
    artistsNextBtn.addEventListener('click', async () => {
      // No limit check for unlimited pagination
      artistsCurrentPage++;
      await loadPageData('artists', artistsCurrentPage - 1, (page) => loadArtistsPage(page, false));
      updatePaginationUI('artists', artistsCurrentPage);
...
// Helper function to update currentSongIndex and sync with window
function setCurrentSongIndex(index) {
  currentSongIndex = index;
  window.currentSongIndex = index;
}

// Helper function to update songs array and sync with window
function setSongs(songsArray) {
  songs = songsArray;
  window.songs = songsArray;
}
...
async function fetchAllHomeData() {
  ...
  try {
    ...
    const popularArtistsResponse = await fetch(
      `${API_BASE_URL}${ENDPOINTS.popularArtists}&n=10`
    );
    if (!popularArtistsResponse.ok) throw new Error("Failed to fetch popular artists");
    const popularArtistsData = await popularArtistsResponse.json();

    if (popularArtistsData?.success && popularArtistsData?.data?.results) {
      renderPopularArtists(popularArtistsData.data.results);
    } else {
      console.warn("Popular Artists API did not return expected data.");
...
let isLoading = false;
  ...
  async function loadSong(songOrIndex) {
    try {
      ...
      if (typeof songOrIndex === 'number') {
        setCurrentSongIndex(songOrIndex);
      } else {
  ...
  function togglePlay() {
    console.log('Main app togglePlay called. isPlaying:', isPlaying, 'Audio paused:', audio ? audio.paused : 'no audio');
  ...
  async function nextSong() {
    ...
    // Check if we need to auto-fetch next page
    await autoFetchNextPageIfNeeded();
  ...
  function initializeVolumeControls() {
    ...
    if (miniVolumeSlider) {
      ...
      safeAddListener(miniVolumeSlider, 'input', (e) => {
        ...
        if (window.playerControls && window.playerControls.updateVolumeIcon) {
          window.playerControls.updateVolumeIcon(volume);
        }
    ...
    if (miniVolumeButton) {
      safeAddListener(miniVolumeButton, 'click', () => {
        if (audio.volume > 0) {
          ...
          if (window.playerControls && window.playerControls.updateVolumeIcon) {
            window.playerControls.updateVolumeIcon(0);
          }
          ...
          if (window.playerControls && window.playerControls.updateVolumeIcon) {
            window.playerControls.updateVolumeIcon(0.8);
          }
  ...
  // Helper function to update all play button icons - use centralized function
  function updatePlayButtonIcons(playing) {
    if (window.playerControls && window.playerControls.updatePlayButtons) {
      window.playerControls.updatePlayButtons(playing);
    }
  }
  ...
  // Mini Player
  safeAddListener(miniPlayButton, 'click', () => {
    console.log('Mini play button clicked');
    
    // First try to use the centralized player controls
    if (window.playerControls && window.playerControls.togglePlay) {
      console.log('Using window.playerControls.togglePlay');
      window.playerControls.togglePlay();
    } 
    // Fallback to the main app togglePlay function
    else if (typeof togglePlay === 'function') {
      console.log('Using main app togglePlay');
      togglePlay();
    }
    // Last resort - try to load and play first available song
    else {
      console.log('Using direct song loading');
      if (songs && songs.length > 0) {
        console.log('Loading first available song:', songs[0].title || songs[0].name);
        ...
        showNotification('No songs available. Please browse and select music first.', 'warning');
      }
    }
  });
    ...
    switch(e.code) {
      ...
      case 'ArrowUp':
        ...
        if (audio && audio.volume < 1) {
          ...
          // Update icons
          if (window.playerControls && window.playerControls.updateVolumeIcon) {
            window.playerControls.updateVolumeIcon(newVolume);
          }
  ...
  function renderPopularArtists(artistsData) {
    ...
    artistsData.forEach(item => {
      // API might return charts, find artists in them or handle chart objects
      const card = createMediaCard(item, 'artist');
      container.appendChild(card);
    });
  }
  ...
  function hideAllPages() {
    ...
    const pages = ['favorites-page', 'playlists-page'];
...
async function loadArtistsPage(page = 0, appendMode = false) {
  console.log('loadArtistsPage called with page:', page, 'appendMode:', appendMode);
  const container = document.getElementById('artists-page-container');
  
  if (!container) {
    console.error('artists-page-container not found');
    return;
  }
  ...
  if (!appendMode) {
    ...
    currentPageSongs = [];
    allLoadedSongs = [];
    currentPageType = 'artists';
  }
  
  try {
    // For artists page, we'll use a search query approach with pagination
    const artistQueries = [
      ...
      'tulsi kumar bollywood',
      'asees kaur bollywood'
    ];
    
    // Use page to determine which artist query to use
    const queryIndex = page % artistQueries.length;
    const query = artistQueries[queryIndex];
    
    const response = await fetch(
      `${API_BASE_URL}/api/search/songs?query=${encodeURIComponent(query)}&language=hindi&limit=20&page=${Math.floor(page / artistQueries.length)}`
    );
    ...
    if (data?.success && data?.data?.results && data.data.results.length > 0) {
      ...
      console.log('Total loaded artist songs:', allLoadedSongs.length);
      
      if (!appendMode) {
        showNotification(`Loaded ${songs.length} songs from ${query.split(' ')[0]} ${query.split(' ')[1]}! 🎤`, 'info');
      }
    } else {
      console.log('No more artist songs available for page:', page);
      if (!appendMode) {
        container.innerHTML = '<div class="empty-state"><i class="bi bi-exclamation-triangle"></i><h3>No artist songs found</h3><p>Please try again later.</p></div>';
      } else {
        showNotification('No more artist songs available', 'info');
      }
    }
  } catch (error) {
    console.error('Error loading artists page:', error);
    showNotification('Error loading artist songs', 'error');
  ...
  card.addEventListener('click', () => {
    if (isAlbum) {
      ...
      if (isSearchResult || isGridLayout) {
        ...
        // For regular cards, use the provided index
        currentSongIndex = index;
        loadSong(index).then(() => {
          playSong();
        });
...
function initializeHeroCTA() {
  ...
  if (startListeningBtn) {
    startListeningBtn.addEventListener('click', function() {
      // Scroll to trending section and start playing
  ...
  if (playTrendingBtn) {
    playTrendingBtn.addEventListener('click', function() {
      // Shuffle and play trending songs
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  initializeHeroCTA();
  initializeKeyboardShortcuts();
  initializeEnhancedFullscreenPlayer(); // Use enhanced version
